...
// ============================================================================
/**
 * Composition Over Inheritance - Flexible code organization (ES6+)
 *
 * Characteristics:
...
console.log("\n=== 11. Composition Over Inheritance Demo ===");

// 11.1 Problem: Deep inheritance
console.log("\nProblem with deep inheritance:");
console.log("  Animal → Mammal → Flying → Bat");
console.log("  Animal → Fish → Swimming → Dolphin");
console.log("  Can lead to 'class explosion'");

// 11.2 Solution: Composition
class Engine {
...
class Wheels {
  ...
  rotate() { return `${this.count} wheels rotating`; }
}
...
// Compose a car from components
class Car {
  constructor(name, engine, wheels, gps = null) {
    this.name = name;
    this.engine = engine;
    this.wheels = wheels;
    this.gps = gps;
...
const sportsCar = new Car("Sports Car", v8Engine, fourWheels, gps);
sportsCar.start();

// 11.3 Behavior composition with classes
...
const CanWalk = (superclass) => class extends superclass {
...
class BehaviorDuck extends CanWalk(CanSwim(CanFly(BaseAnimal))) {
  constructor() {
    super("Duck");
  }
}
...
const behaviorDuck = new BehaviorDuck();
behaviorDuck.fly();
behaviorDuck.swim();
behaviorDuck.walk();
...
// ============================================================================
// 12. STRATEGY PATTERN WITH CLASSES
// ============================================================================
/**
 * Strategy Pattern - Interchangeable algorithms (ES6+)
 *
 * Characteristics:
...
console.log("\n=== 12. Strategy Pattern Demo ===");

// 12.1 Payment strategies
...
class BitcoinPayment extends PaymentStrategy {
  ...
  pay(amount) {
    console.log(`  Paid $${amount} in BTC to ${this.wallet}`);
