...
console.log(
  "  localStorage: persistent across sessions, 5-10MB limit, sync API"
);
...
console.log("\n2. Cookies:");
console.log(
  "  Small text data (4KB limit) sent with every HTTP request to the server"
);
console.log(
...
console.log(
  "  Limitations: small size, sent with every request, security risks if misconfigured"
);
...
console.log(
  "  Use cases: offline apps, large dataset storage, complex queries"
);
console.log(
  "  Comparison: vs localStorage (sync, small) | vs Cookies (sent to server)"
);
...
console.log(
  "  SPA routing core: update URL and UI dynamically without full page refresh"
);
console.log(
  "  State management: store arbitrary state objects with history entries"
);
console.log(
  "  Best practices: always handle direct URL access (server-side fallback), update document.title"
);
console.log(
  "  ⚠️  pushState doesn't trigger popstate; handle initial load separately"
);
...
console.log(
  "  Patterns: timeout, request cancellation (AbortController), retries, API wrappers"
);
console.log(
  "  📘 See 33.3-fetch-practical-patterns.js for complete implementation examples"
);
...
console.log(
  "  Events: open, message, error, close; Methods: send(data), close()"
);
console.log("  Use cases: real-time chat, live updates, collaborative editing");
console.log(
  "  ⚠️  Use wss:// (secure) in production; implement heartbeat/reconnection logic"
);
...
console.log("\n7. Server-Sent Events (SSE):");
console.log(
  "  One-way server-to-client communication over HTTP, automatic reconnection"
);
console.log("  API: const es = new EventSource('/events');");
console.log(
  "  Events: 'open', 'message', 'error', custom events; Methods: close()"
);
console.log(
  "  Use cases: real-time notifications, live feeds, server-sent updates"
);
console.log(
  "  Comparison: vs WebSocket (bidirectional) | vs polling (lower overhead)"
);
...
console.log(
  "  Use cases: sync state across tabs, notify other tabs of login/logout"
);
...
console.log("\n=== Common Pitfalls ===");
console.log(
  "❌ Storing sensitive data in localStorage: vulnerable to XSS attacks"
);
console.log(
  "❌ Forgetting JSON.stringify/parse when storing objects in storage APIs"
);
console.log("❌ Not handling storage quota exceeded errors");
console.log(
  "❌ Using Cookies for large data: wastes bandwidth on every request"
);
console.log("❌ Not implementing reconnection logic for WebSocket/SSE");
console.log(
  "❌ Not canceling fetch requests when they're no longer needed (memory leaks)"
);
console.log(
  "❌ Using pushState without server-side routing fallback (404 on direct access)"
);
...
console.log("\n=== Best Practices ===");
console.log(
  "✅ Use HttpOnly + Secure + SameSite=Strict cookies for authentication tokens"
);
console.log(
  "✅ Implement proper error handling and fallback for all storage APIs"
);
console.log(
  "✅ Use IndexedDB for large/offline data; localStorage for small preferences"
);
console.log("✅ Add AbortController timeouts to all network requests");
console.log(
  "✅ Use secure protocols (wss://, https://) for all network communication"
);
console.log(
  "✅ Sanitize all data received from network/storage before rendering"
);
console.log(
  "✅ Clean up event listeners and close connections when no longer needed"
);
...
console.log("\n=== Cross-references ===");
console.log(
  "📘 33-fetch-api.js / 33.3-fetch-practical-patterns.js - Complete Fetch API patterns"
);
console.log("📘 37-events.js - DOM event handling fundamentals");
console.log(
  "📘 48-security.js - Web security best practices for storage/network"
);
