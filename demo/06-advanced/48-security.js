...
console.log("  DOMPurify: Sanitizes untrusted HTML - import DOMPurify from 'dompurify'; const clean = DOMPurify.sanitize(dirty);");
...
console.log("  ⚠️  Avoid: innerHTML with user input, eval(), document.write(), inline event handlers with untrusted data");
...
// ✅ Protection Methods:
console.log("  1. CSRF Tokens: Server-generated token included in request headers/forms, validated server-side");
console.log("  2. SameSite Cookies: Strict (same-site only), Lax (top-level navigation), None (cross-site, requires Secure)");
...
// ✅ Implementation:
console.log("  - Meta tag: <meta http-equiv=\"Content-Security-Policy\" content=\"default-src 'self';\">");
console.log("  - HTTP Header: Content-Security-Policy: default-src 'self'; script-src 'self' 'nonce-{random}';");

console.log("\n  Common directives:");
console.log("  default-src (fallback), script-src, style-src, img-src, connect-src, frame-ancestors (clickjacking protection)");
console.log("  Nonce: Random per-request value for allowing specific inline scripts/styles");
console.log("  Reporting: Use report-uri / report-to to collect violation reports without enforcing");
...
console.log("  2. Encryption: Use Web Crypto API to encrypt sensitive data if must store client-side");
console.log("  3. Secure attributes: Always use secure: true (HTTPS only), sameSite: strict, short maxAge");
...
console.log("  Command Injection: Never pass user input to shell commands; use safe APIs with parameterized inputs");

// ============================================
// 6. Web Crypto API
// ============================================
console.log("\n6. Web Crypto API:");
console.log("  Native browser/Node API for cryptographic operations: hashing, encryption, signing, key generation");

console.log("\n  Common use cases:");
console.log("  - Hashing: SHA-256 for password hashing (use with salt, 100k+ iterations via PBKDF2)");
...
console.log("  - Random number generation: crypto.getRandomValues() for secure randomness (never Math.random())");

console.log("\n  Example: const hash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode('data'));");
console.log("  ⚠️  Never implement custom cryptography; use standard algorithms and well-audited libraries");
...
console.log("  A02: Cryptographic Failures - Use strong, up-to-date algorithms, never hardcode secrets");
...
console.log("  A05: Security Misconfiguration - Keep systems patched, remove debug features, use secure defaults");
console.log("  A07: Identification and Authentication Failures - Use MFA, implement secure session management, rate limit login attempts");
console.log("  A08: Software and Data Integrity Failures - Verify dependencies, use subresource integrity (SRI) for CDN resources");
console.log("  A10: Server-Side Request Forgery (SSRF) - Validate and restrict outbound requests from server");
...
// Trusted Types
console.log("  Trusted Types: Prevent DOM XSS by enforcing type checks for dangerous sink functions (innerHTML, eval, etc.)");
console.log("  Enable via CSP: Content-Security-Policy: require-trusted-types-for 'script';");

// COOP/COEP
console.log("  Cross-Origin Isolation: COOP (Cross-Origin Opener Policy) + COEP (Cross-Origin Embedder Policy)");
console.log("  Isolates your site from cross-origin documents, enables SharedArrayBuffer and high-resolution timers");
...
console.log("✅ Use secure defaults: Features should be secure by default, opt-in for less secure behavior");
